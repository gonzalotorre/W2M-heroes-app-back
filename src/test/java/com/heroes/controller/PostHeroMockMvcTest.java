...
public class PostHeroMockMvcTest {
    ...
    public void postHero_200() throws Exception {
        final var heroDTO = new HeroDTO(1, "Spider-Man", "throws web");

        Mockito.when(this.heroesServiceMock.save(heroDTO)).thenReturn(heroDTO.getId());
        ...
        verify(this.heroesServiceMock).save(heroDTO);
    }
    ...
    @Test
    @WithMockUser(username = "user", password = "pass", authorities = {"USER_ROLE"})
    public void postHero_404() throws Exception {
        this.mockMvc.perform(post("/sav")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
    ...
    @Test
    public void postHero_401() throws Exception {
        this.mockMvc.perform(post("/save")
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
